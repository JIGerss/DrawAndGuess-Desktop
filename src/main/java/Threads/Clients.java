...
public class Clients extends PApplet implements Runnable {
    public static final int WIDTH = 900, HEIGHT = 710, CANVAS_WIDTH = 350, CANVAS_HEIGHT = (int) (CANVAS_WIDTH / 9.0 * 18.0);
    private final float leftX = (float) (WIDTH / 2 - CANVAS_WIDTH / 2), UpY = 8;
    ...
    public void setup() {
        ...
        URL = convertMD5(URL);
        setVariables();
        logout = new Structures.Button(665, 530, 200, 50);
    ...
    private void setVariables() {
        ...
        if (!isDrawer) {
            offlineLines = getLines();
            HttpRequest.doPost(URL + "games/" + gameId + "/join", "", JSON.toJSONString(Player));
        }
        players = getUsersInGame();
