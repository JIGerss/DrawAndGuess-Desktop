...
public class Lobby extends PApplet {
    ...
    private Structures.Button logout;
    private User Player;
    private Game[] games;

    public static void main(String[] args) {
        PApplet.main("Threads.Lobby");
    }
    ...
    public void setup() {
        ...
        setVariables();
        vocabs = getVocabs();
        logout = new Structures.Button(665, 430, 200, 50);
    ...
    public void draw() {
        ...
        for (Room room : rooms) {
            if (isMovedOnButton(room.button)) fill(190, 163, 162);
            else {
                if (!room.isGame) fill(230, 198, 26);
                else fill(200, 198, 26);
            }
            ...
            } else text("        空", room.button.x - 100, room.button.y + 59);
        }
        if (isMovedOnButton(logout)) fill(80, 80, 80);
        else fill(24, 25, 28);
    ...
    private boolean isMovedOnButton(Structures.Button button) {
    ...
    private void setRooms() {
        for (int i = 0; i < rooms.length; i++) {
            if (gameNum > i) {
                rooms[i] = new Room(new Structures.Button(130 + (i % 2) * 300, 85 + (i / 2) * 150, 100, 100), games[i]);
